...
Install: `npm install better-sqlite3` and `npm install -D @types/better-sqlite3`
  ...
  <verify>
Run `node -e "const {getDb} = require('./lib/db.ts'); console.log(getDb().prepare('SELECT name FROM sqlite_master WHERE type=\"table\"').all())"`

Should output User table exists.
  </verify>
  <done>
...
Install: `npm install bcrypt jsonwebtoken` and `npm install -D @types/bcrypt @types/jsonwebtoken`
  ...
  <verify>
Run `node -e "const {hashPassword, comparePassword} = require('./lib/auth.ts'); hashPassword('test').then(h => comparePassword('test', h)).then(console.log)"`
